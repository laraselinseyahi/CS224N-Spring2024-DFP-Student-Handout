...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    # prefix tuning embeddings
    prefix_embeds = self.config.prefix_embeddings(torch.arange(self.config.prefix_length, device=input_ids.device))
    prefix_embeds = prefix_embeds.unsqueeze(0).repeat(input_shape[0], 1, 1)

    combined_embeds = torch.cat([prefix_embeds, inputs_embeds + pos_embeds + tk_type_embeds], dim=1)

    # Add three embeddings together; then apply embed_layer_norm and dropout and return.
    # embed_layer_norm_applied = self.embed_layer_norm(inputs_embeds + pos_embeds + tk_type_embeds)
    embed_layer_norm_applied = self.embed_layer_norm(combined_embeds)
  ...
  def forward(self, input_ids, attention_mask):
    ...
    # prefix tuning addition
    extended_prefix_mask = torch.zeros((input_ids.shape[0], self.config.prefix_length), device=input_ids.device, dtype=attention_mask.dtype)
    # new_attention_mask = torch.cat([extended_prefix_mask, attention_mask], dim=1)
    new_attention_mask = torch.cat([extended_prefix_mask, attention_mask], dim=1)
    ...
    sequence_output = self.encode(embedding_output, attention_mask=new_attention_mask)
