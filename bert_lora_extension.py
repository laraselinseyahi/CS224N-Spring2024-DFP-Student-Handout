...
class BertSelfAttention(nn.Module):
  def __init__(self, config):
    ...
    self.alpha = 32  # Scaling factor for LoRA
    self.scaling = self.alpha / self.rank  # Alpha scaling factor
  ...
  def transform_lora(self, x, linear_layer, lora_A, lora_B):
    ...
    lora_proj = torch.matmul(lora_proj, lora_A) * self.scaling
...
class BertLayer(nn.Module):
  def __init__(self, config):
    ...
    # LoRA initialization for feedforward layers
    self.rank = 4  # Rank of the low-rank matrices
    self.alpha = 32  # Scaling factor for LoRA
    self.scaling = self.alpha / self.rank  # Alpha scaling factor

    self.lora_A_interm = nn.Parameter(torch.randn(self.rank, config.intermediate_size))
    self.lora_B_interm = nn.Parameter(torch.zeros(config.hidden_size, self.rank))
    self.lora_A_out = nn.Parameter(torch.randn(self.rank, config.hidden_size))
    self.lora_B_out = nn.Parameter(torch.zeros(config.intermediate_size, self.rank))

  def transform_lora(self, x, linear_layer, lora_A, lora_B):
        proj = linear_layer(x)

        # LoRA adaptation with scaling
        lora_proj = torch.matmul(x, lora_B)
        lora_proj = torch.matmul(lora_proj, lora_A) * self.scaling
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer, lora_A=None, lora_B=None):
    """
    ...
    transformed = self.transform_lora(output, dense_layer, lora_A, lora_B) if lora_A is not None and lora_B is not None else dense_layer(output)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    feed_forward_1 = self.transform_lora(added_norm, self.interm_dense, self.lora_A_interm, self.lora_B_interm)
    feed_forward_2 = self.interm_af(feed_forward_1)

    add_norm_2 = self.add_norm(added_norm, feed_forward_2, self.out_dense, self.out_dropout, self.out_layer_norm, self.lora_A_out, self.lora_B_out)
    return add_norm_2
    # raise NotImplementedError
